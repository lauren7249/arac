...
class processLinkedIn(MRJob):
    ...
    def mapper(self, _, filename):
        ...
        writefiles = mgr.list(["education_file.txt", "job_file.txt", "person_file.txt"])
        ...
        bucket = s3conn.get_bucket(Global.write_bucket)  
        key = bucket.get_key("processed/linkedin_id_2_prospect_id.dict")
        key.get_contents_to_filename('linkedin_id_2_prospect_id.dict')

        linkedin_id_2_prospect_id = mgr.dict(joblib.load('linkedin_id_2_prospect_id.dict'))

        pool = multiprocessing.Pool(cpu_count-1)
        df = pandas.read_csv("https://s3.amazonaws.com/mrjob-lists/" + filename, delimiter="\t", names=["num", "key"])
        for index, row in df.iterrows():
            file_to_parse = row["key"]
            pool.apply_async(parseFile, (file_to_parse, Global, linkedin_id_2_prospect_id, writefiles, educations_lock, jobs_lock, prospects_lock, good_degrees_dict, good_schools_dict, good_titles_dict, good_companies_dict))
...
def parseFile(s3_key, Global, linkedin_id_2_prospect_id, writefiles, educations_lock, jobs_lock, prospects_lock, good_degrees_dict, good_schools_dict, good_titles_dict, good_companies_dict):
    try:
        ...
        prospect_id = linkedin_id_2_prospect_id.get(linkedin_id)
