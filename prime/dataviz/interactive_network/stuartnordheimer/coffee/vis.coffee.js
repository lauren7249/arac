...
Network = () ->
  ...
  .range(["#FFFFFF","#51BDF6","#2881BA", "#0F4C8B"]);  
  # tooltip used to display details
  tooltip = Tooltip("vis-tooltip", 230)

  # charge used in Entity layout
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      Entities = sortedEntities(curNodesData, curLinksData)
      updateCenters(Entities)
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.salary))
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.connections)
  ...
  # attr is value stored in node, like 'Entity'
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      connectionss = allNodes.map((d) -> d.connections).sort(d3.ascending)
      cutoff = d3.quantile(connectionss, 0.5)
      filteredNodes = allNodes.filter (n) ->
        if filter == "popular"
          n.connections > cutoff
        else if filter == "obscure"
          n.connections <= cutoff

    filteredNodes

  # Returns array of Entities sorted based on
  # current sorting method.
  sortedEntities = (nodes,links) ->
    Entities = []
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.Entity] ?= 0
        counts[l.source.Entity] += 1
        counts[l.target.Entity] ?= 0
        counts[l.target.Entity] += 1
      # add any missing Entities that dont have any links
      nodes.forEach (n) ->
        counts[n.Entity] ?= 0

      # sort based on counts
      Entities = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      Entities = Entities.map (v) -> v.key
    else
      # sort Entities by Person count
      counts = nodeCounts(nodes, "Entity")
      Entities = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      Entities = Entities.map (v) -> v.key
  ...
  updateCenters = (Entities) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(300).increment(18).keys(Entities)
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.salary))
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.Entity)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.salary)).darker().toString()

  # Mouseover tooltip function
  showDetails = (d,i) ->
    content = ""
    if d.name 
      content += '<p class="main">' + d.name + '</span></p><hr class="tooltip-hr">'
    if d.school_name
      content += '<p class="main">' + d.school_name + '</span></p><hr class="tooltip-hr">'
    if d.company_name
      content += '<p class="main">' + d.company_name + '</span></p><hr class="tooltip-hr">'    
    if d.image_url
      content += '<p class="main" >' + '<img src="' + d.image_url + '">' +'</span></p>'
